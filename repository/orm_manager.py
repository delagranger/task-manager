...
class ORMManager:
    ...
    def add_task(self, task: Task) -> tuple[int, str, str, str]:
        with session_scope(self.Session) as session:
            found_group = (
                ...
                .filter(GroupModel.title == task.group)
                .first()
            )
            if not found_group:
                raise GroupNotFound(task.group)
            task_orm = TaskModel(
                title=task.title, status=task.status, group=found_group
            )
            session.add(task_orm)
            session.flush()
            log.info(
                "Add task: SUCCESS; ID=%r, Title=%r, Status=%r, Group=%r",
                task_orm.id, task_orm.title, task_orm.status, task_orm.group,
            )
            return (
                task_orm.id,
                task_orm.title,
                task_orm.status,
                task_orm.group.title,
            )
    ...
    def add_group(self, group: Group) -> tuple[int, str]:
        with session_scope(self.Session) as session:
            query = session.query(GroupModel)
            existing_group = (
                query.filter(GroupModel.title == group.title).first()
            )
            if existing_group:
            ...
            session.flush()
            log.info(
                "Add group: SUCCESS; ID=%r, Title=%r",
                group_orm.id, group_orm.title,
            )
    ...
    def list_groups(self, sort_type: str) -> list[Group]:
        ...
        with session_scope(self.Session) as session:
            ...
            rows = query.all()
            log.debug(
                "Collect and sort groups: SUCCESS; Sort type = %r",
                sort_type,
            )
            groups = []
            for g in rows:
    ...
    def delete_group(self, id: int) -> int:
        with session_scope(self.Session) as session:
            ...
            default_group = (
                query.filter(GroupModel.title == DEFAULT_GROUP_TITLE).first()
            )
    ...
    def patch_task(
        ...
        with session_scope(self.Session) as session:
            ...
            new_task = (
                task.id, task.title, task.status, task.group.title
            )
        log.info(
            "Format task: SUCCESS; "
            "ID=%r, New title=%r, New status=%r, New group=%r",
            new_task[0], new_task[1], new_task[2], new_task[3],
        )
        return new_task
    ...
    def patch_group(
        ...
        with session_scope(self.Session) as session:
            ...
            if title is not None:
                existing_group = (
                    query.filter(GroupModel.title == title).first()
                )
                if existing_group:
        ...
        log.info(
            "Format group: SUCCESS; ID=%r, New title=%r",
